...
<body class="bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen" x-data="showroomApp()"
    ...
    <div class="fixed bottom-0 left-0 right-0 bg-gradient-to-t from-white via-white to-transparent pt-6 pb-4 px-4 z-50">
        <div class="container mx-auto max-w-lg">
            <button @@click="sendWhatsAppOrder()" {{ $whatsappConfigured ? '' : 'disabled' }}
                ...
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-lg" x-text="cartItemsCount > 0 ? 'إرسال الطلب عبر واتساب' : 'تواصل معنا عبر واتساب'"></span>
                </div>
                <div x-show="cartItemsCount > 0" class="bg-white text-green-600 font-bold px-3 py-1 rounded-full text-sm">
    ...
    <script>
        function showroomApp() {
            return {
                ...
                sendWhatsAppOrder() {
                    const mode = '{{ $mode }}';
                    const modeLabel = mode === 'retail' ? 'قطاعي' : 'الجملة';
                    let message = '';

                    if (Object.keys(this.cart).length === 0) {
                        // Send a general inquiry when cart is empty
                        message = `مرحباً، أود الاستفسار عن منتجات كتالوج ${modeLabel}.\n\nشكراً لكم.`;
                    } else {
                        // Send specific order when cart has items
                        message = `مرحباً، أود طلب المنتجات التالية من كتالوج ${modeLabel}:\n\n`;
