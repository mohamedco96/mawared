...
/**
 * Sales Returns Import Seeder
 *
 * This seeder imports sales returns from legacy CSV files:
 * - sales_return_headers.csv (Source: seqr.csv)
 * - sales_return_items.csv (Source: return.csv)
 *
 * Mapping:
 * - Header: no_s -> return_number (prefixed with 'SR-'), to -> partner_id, da_s -> date, kst -> total
 ...
 * - Handles missing products/customers gracefully with fallbacks
...
class SalesReturnsImportSeeder extends Seeder
{
    ...
    private int $missingCustomers = 0;
    private array $missingProductNames = [];
    private array $missingCustomerNames = [];

    public function run(): void
    {
        $this->command->info('🚀 Starting Sales Returns Import...');
        ...
        $this->command->info('💾 Processing sales return headers...');
        $this->processReturnHeaders($itemsGrouped);

        // Display summary
        $duration = round(microtime(true) - $startTime, 2);
        $this->command->newLine();
        $this->command->info('✅ Sales Returns Import Complete!');
        $this->command->table(
            ['Metric', 'Value'],
            [
                ...
                ['Missing Customers', number_format($this->missingCustomers)],
                ['Duration', "{$duration}s"],
        ...
        // Show sample missing customers
        if (!empty($this->missingCustomerNames)) {
            $this->command->newLine();
            $this->command->warn('⚠️  Sample Missing Customers (first 20):');
            foreach (array_slice($this->missingCustomerNames, 0, 20) as $name) {
                $this->command->line('   - ' . $name);
            }
        }
    }
    ...
    {
        $csvPath = database_path('seeders/data/sales_return_items.csv');
    ...
    {
        $csvPath = database_path('seeders/data/sales_return_headers.csv');
        ...
        while (($row = fgetcsv($handle)) !== false) {
            ...
            // Extract fields (already UTF-8 encoded)
            $returnNo = isset($row[6]) ? $this->cleanScientificNotation(trim($row[6])) : null;
            $customerName = isset($row[1]) ? trim($row[1]) : null;
            ...
            $customerId = null;
            if ($customerName) {
                $customerId = $this->customerMap[$customerName] ?? $this->customerMap[$this->normalizeName($customerName)] ?? null;
            }

            if (!$customerId) {
                // Track missing customers
                if ($customerName && !in_array($customerName, $this->missingCustomerNames)) {
                    $this->missingCustomerNames[] = $customerName;
                    $this->missingCustomers++;
                }
                // Use walk-in customer as fallback
                $customerId = $this->walkInCustomerId;
            }
    ...
    {
        // Build return number with SR- prefix
        $returnNumber = 'SR-' . $data['return_no'];

        // Check if return already exists (idempotency)
        if (SalesReturn::where('return_number', $returnNumber)->exists()) {
            $this->skippedReturns++;
            return;
        }
        ...
        $return = SalesReturn::create([
            ...
            'sales_invoice_id' => null, // No direct link to invoice in source data
